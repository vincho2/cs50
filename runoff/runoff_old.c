  ...
  /*  for (int i = 0; i < voter_count; i++)
    ...
    while (true)
    {
        ...
        for (int i = 0; i < candidate_count; i++)
        {
            ...
            printf("elimnated ?: %i / ", candidates[i].eliminated);
...
void tabulate(void)
{
    ...
    // Loop on each voter to define its preferred remaining candidate
    for (int i = 0; i < voter_count; i++)
    {
        // Initialize the minimum rank found with the total number of candidates
        int min_rank = candidate_count;
        // Loop on each candidate and identify lowest "live" rank
        for (int j = 0; j < candidate_count; j++)
        {
            // Define the lowest rank among remaining candidates for the current voter
            if (!candidates[j].eliminated)
            {
                if (preferences[i][j] < min_rank)
                {
                    min_rank = preferences[i][j];
                }
            }
        }
        // Loop on each candidate again and vote for the preferred candidate for the current voter
        for (int j = 0; j < candidate_count; j++)
        {
            if (preferences[i][j] == min_rank)
            {
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
            // Keep count of the number of remaining candidates
